...
if(!empty($action)) {
	switch($action) {
		case "add":
			$result = mysql_query("INSERT INTO comments(comments) VALUES('".$_POST["txtmessage"]."')");
			if($result){
...
<button class="btnDeleteAction" name="delete" onClick="callCrudAction(\'delete\',' . $insert_id . ')">Delete</button>
		...
		case "edit":
			$result = mysql_query("UPDATE comments set message = '".$_POST["txtmessage"]."' WHERE  id=".$_POST["message_id"]);
			if($result){
		...
		case "delete": 
			if(!empty($_POST["message_id"])) {
				mysql_query("DELETE FROM comments WHERE id=".$_POST["message_id"]);
			}
			break;
	}
}
